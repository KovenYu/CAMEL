  ...
  methods
    ...
    function obj = Conv(varargin)
      ...
      switch obj.initMethod
          case 'gaussian'
          case 'one'
              assert(obj.size(1) == 1 && obj.size(2) == 1, ...
                  ['filter widths must be 1 for initMethod = ''one''']);
              assert(obj.size(3) == obj.size(4), ...
                  ['# i/o channels must be the same for initMethod = ''one''']);
          otherwise
              error('unknown initMethod.');
      end
    end
  end
end
