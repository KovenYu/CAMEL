function str = printfig(obj, inputSizes, varargin)
...
if exist('digraph') == 2
    ...
    layerNames = {net.layers.name};
    nodeNames = {net.vars.name};
    layer_count = numel(layerNames);
    node_count = numel(nodeNames);
    A = zeros(node_count);
    for li = 1:layer_count
        l = net.layers(li);
        inputs=l.inputs;
        name=l.outputs;%l.name;
        A(:,find_connection(name,nodeNames)) = find_connection(inputs,nodeNames);
    end
    g = biograph(A,nodeNames);
    h=view(g);
end
end

function ind = find_connection(inputs,nodeNames)
    if ~iscell(inputs)
        inputs={inputs};
    end
    ind = false(length(nodeNames),1);
    for i=1:length(inputs)
        for j=1:length(nodeNames)
            if strcmp(inputs{i},nodeNames{j})
                ind(j)=true;
            end
        end
    end
